...
function LRNewViewModel(parent){
	...
	this.showmorebtn = ko.observable(true);
	...
	this.choiceChange = function () {
	    if (typeof self.leavetype() !== "undefined") {
	        // More btn
	        var overonedayarr = ["ลาคลอด", "ลาบวช"];
	        if (overonedayarr.indexOf(self.leavetype()[0]) > -1) {
	            self.fromdatetype("เต็มวัน");
	            self.moretemplate("idmorethenone");
	            self.moreModel(new moreThenOne(true));
	            self.morebool(true);
	            self.showmorebtn(false);
	        }
	        else {
	            if (self.morebool() && !self.showmorebtn()) {
	                self.moretemplate("idwating");
	                self.moreModel(new moreThenOne(false));
	                self.morebool(false);
	            }
